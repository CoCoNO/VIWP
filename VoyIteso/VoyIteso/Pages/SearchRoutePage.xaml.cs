...
{
    public partial class SearchRoutePage : PhoneApplicationPage
    {
        bool canChangeState;
        bool isSearchTerm;
        bool isOrigin;
        ...
        */
        /*
        #region ResultRoutePanelTap
        
        private void ResultRoutePanel_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            StackPanel s = (StackPanel)sender;
            NavigationService.Navigate(new Uri("/Pages/InfoRoute.xaml?msg=" + s.Tag,UriKind.Relative));
        }
        ...
        */
        ...
        {
            SendLiftRequest(currentRoute.route_id);
        }
        ...
        {
            apiConnector.UpdateUrl(AppResources.ApiSearchRoute);
            apiConnector.setParametersUrl(
                "origen=" + txtOrigin.Text + 
                "&distancia=0.045" +
                "&destino=" + txtDestiny.Text +
                "&distancia_destino=0.045" +
                "&fecha=" + dateString +
                "&fumador=" + intSmoker +
                "&rol=-1" + 
                "&genero=" + intGender +
                "&calificacion=-1" +
                "&latitud_destino=" + maping.locationB.GeoCoordinate.Latitude +
                "&longitud_destino=" + maping.locationB.GeoCoordinate.Longitude +
                "&latitud_origen=" + maping.locationA.GeoCoordinate.Latitude +
                "&longitud_origen=" + maping.locationB.GeoCoordinate.Longitude +
                "&hora=" + timeString +
        ...
        void apiConnector_exceptionChanged(object sender, EventArgs e)
        {
            if (apiConnector.throwException != null || apiConnector.throwException != "")
            {
                Dispatcher.BeginInvoke(() =>
                {
                    ...
                    {
                        MessageBox.Show("Un error se produjo durante la busqueda. Vuelve a intenntarlo", "Error", MessageBoxButton.OK);
                    }
                });
        ...
        private void SendLiftRequest(int id)
        {
            apiConnector.UpdateUrl(AppResources.ApiAskForLift);
            apiConnector.setParametersUrl(
                "ruta_id=" + id +
                "&mensaje_solicitud=temporal" +
                "&fecha_solicitud=" + string.Format("{0:dd-MM-yyyy}", DateTime.Now) +
                "&security_token=" + user.Token);
            progress.showProgressIndicator(this, "Enviando");
            apiConnector.SendPostRequest();
